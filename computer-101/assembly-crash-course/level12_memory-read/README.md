# Level Description
```
hacker@assembly-crash-course~memory-read:~/asm_courses$ /challenge/run memory-read.elf 
...
# Solution
```
python memory-read.py
```
OR 

```
as memory-read.s -o memory-read.o; ld memory-read.o -o memory-read.elf; /challenge/run memory-read.elf
```

# Flag
`pwn.college{EVt2HBl_uVEHrs-hFoPB5JazCNB.dJTM4MDL5YTNzEzW}`
